...
- **纯净绿色便携模式 (True Portable Mode)**：绿色压缩包内置 \`portable.flag\`，运行时自动将 SQLite 数据库与 AI 凭据保存在同级 \`./data/\` 目录，完全离线、零注册表残留、U 盘即插即用。
- **本地 LLM 安全代理**：全面移除渲染层网络直连，所有 AI 参谋请求由 Rust 本地后端统一中继代理，支持 Google Gemini 原生 REST 协议与 OpenAI 协议双模，安全凭据本地掩码隔离。
- **全方位数据完整性加固**：前置国策依赖图引入 BFS 环路检测，严防死锁成环；强制跨空间数据隔离；特质装备状态平滑迁移。
